...
type ProductData = {
  ...
  const getMarketData = (category: string): MarketData => {
  ...
  export const optimizePrice = (product: ProductData, similarProducts: ProductData[] = []): PriceRecommendation => {
    const marketData = getMarketData(product.category)
    ...
    if (similarProducts.length > 0) {
      ...
      if (product.price > avgCompetitorPrice * 1.2) {
        ...
        reasoning.push(
          `Your price (Rs. ${product.price}) is significantly higher than similar products (avg: रू ${avgCompetitorPrice.toFixed(2)}). Consider lowering your price to be more competitive.`,
        )
    ...
    if (product.price < marketData.minPrice) {
      reasoning.push(`Your price is below the typical minimum price (Rs. ${marketData.minPrice}) for ${product.category}.`)
    } else if (product.price > marketData.maxPrice) {
      reasoning.push(`Your price is above the typical maximum price (Rs. ${marketData.maxPrice}) for ${product.category}.`)
    } else {
      reasoning.push(
